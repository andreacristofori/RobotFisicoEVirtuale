...
let end = content.indexOf('  const translateStatement = (stmt: string): string => {', start);
end = content.lastIndexOf('};', end) + 2;
...
const wrapper = "export const transpilePythonToJsWrapper = (pythonCode: string) => {\n" +
  ...
  "}";
fs.writeFileSync('temp_transpiler_run2.ts', wrapper);

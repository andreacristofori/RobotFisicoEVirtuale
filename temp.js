...
export const transpilePythonToJsWrapper = (pythonCode) => {
  ...
  const transpilePythonToJs = (pythonCode) => {
    ...
    // Extract variables
    const varMatches = pythonCode.match(/^[a-zA-Z_][a-zA-Z0-9_]*\s*=\s*/gm);
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (translated === 'while True:') {
        ...
        currentIndent = indent + 4;
      } else if (translated.startsWith('if ') && translated.endsWith(':')) {
        const cond = translated.substring(3, translated.length - 1);
        ...
        const stmt = translated.substring(4).trim();
  ...
  const runSimulationCode = async () => {
    ...
    const runMotor = (port, speed: number) => {
      if (execId !== activeExecutionId.current) return;
    ...
    const runMotorForDegrees = async (port, degrees: number, speed: number) => {
      if (execId !== activeExecutionId.current) return;
      runMotor(port, speed);
    ...
    const showImageLightMatrix = (imageName) => {
      if (execId !== activeExecutionId.current) return;
      robotRef.current.matrixText = '';
    ...
    try {
      ...
      const runnerFn = new AsyncFunction(
        ...
        'int', 'float', 'str', 'len', 'abs', 'round', 'min', 'max',
        `try {
          ${jsCode}
        } catch(e) {
